...
class Bill_App:
    def __init__(self,root):
        ...
        F1=LabelFrame(self.root,bd=10,relief=GROOVE, text="Student Details",font=("times new roman",15,"bold"),fg="gold",bg=bg_color)
        F1.place(x=0,y=80,relwidth=1)

        cname_lbl=Label(F1,bg=bg_color,fg="white",text="Student Name",font=("times new roman",18,"bold")).grid(row=0,column=0,padx=20,pady=5)
        ...
        #cphn_lbl=Entry(F1,textvariable=self.c_phon,width=17,font="arial 15",bd=7,relief=SUNKEN).grid(row=0,column=3,pady=5,padx=10)

        c_bill_lbl=Label(F1,bg=bg_color,fg="white",text="Receipt No.",font=("times new roman",18,"bold")).grid(row=0,column=4,padx=20,pady=5)
        c_bill_lbl=Entry(F1,textvariable=self.search_bill,width=15,font="arial 15",bd=7,relief=SUNKEN).grid(row=0,column=5,pady=5,padx=10)
        ...
        item_no_txt=Entry(F2,width=10,textvariable=self.search_item_name,font=("times new roman",16,"bold"),bd=5,relief=SUNKEN).grid(row=0,column=1,padx=10,pady=10)

        #Button for search using item no.
        search_no_btn=Button(F2,command=self.search_item,bg="White",text="Search",pady=15,width=11,bd=5,font="arial 15 bold").grid(row=1,column=1,padx=5,pady=5)
        ...
        search_name_btn=Button(F3,command=self.add_item,bg="White",text="Add",pady=15,width=11,bd=5,font="arial 15 bold").grid(row=4,column=1,padx=5,pady=5)
        ...
        F5=LabelFrame(self.root,bd=10,relief=GROOVE, text="Receipt area",font=("times new roman",15,"bold"),fg="gold",bg=bg_color)
        ...
        #================Bottom Frame================#
        F6=LabelFrame(self.root,bd=10,relief=GROOVE,text="",font=("times new roman",15,"bold"),bg=bg_color,fg="gold")
        F6.place(x=0,y=545,relwidth=1,height=140)

        btn_F=Frame(F6,bd=7,relief=GROOVE)
        btn_F.place(x=670,width=625,height=105)

        total_btn=Button(btn_F,command=self.find_bill,bg="Blue",text="Total",fg="Black",pady=15,width=9,bd=5,font="arial 15 bold").grid(row=0,column=1,padx=5,pady=5)
        GBill_btn=Button(btn_F,command=self.bill_area,bg="Blue",text="Generate",fg="Black",pady=15,width=12,bd=5,font="arial 15 bold").grid(row=0,column=2,padx=5,pady=5)
        Clear_btn=Button(btn_F,command=self.clear,bg="Blue",text="Clear",fg="Black",pady=15,width=10,bd=5,font="arial 15 bold").grid(row=0,column=3,padx=5,pady=5)
        #Exit_btn=Button(btn_F,command=self.root.destroy,bg="Blue",text="Exit",fg="Black",pady=15,width=10,bd=5,font="arial 15 bold").grid(row=0,column=4,padx=5,pady=5)
        self.welcome_bill()
    ...
    def add_item(self):
        ...
        for each in listOfEntriesInTreeView:
            if self.Stock_Table.item(each)['values'][0] == self.item_no.get():
                ...
                if new_quantity<available_quantity or new_quantity==available_quantity:
                    ...
                    messagebox.showinfo("Alert","Amount entered is more than amount in Inventory.")
                    return
    ...
    def bill_area(self):
        ...
        total=0
        if self.c_name.get()=='':
            messagebox.showinfo("Alert","Enter Student Name first")
            return
        ...
        cur.execute(f"select * from customer where customer_name='{self.c_name.get()}'")
        #and customer_phone={self.c_phon.get()}
        rows=cur.fetchall()
        if len(rows)!=0:
            customer_id=rows[0][0]
        else:
            customer_id=random.randint(1,1000)
            cur.execute(f"insert into customer values ({customer_id},'{self.c_name.get()}')")
            #,{self.c_phon.get()})")
            con.commit()
        ...
        self.txtarea.insert(END,f"\n=======================================")
        self.txtarea.insert(END,f"\n\t\tSubtotal Kes. {total}")
        self.txtarea.insert(END,f"\n\t\tSales Tax Kes. {0.16*total}")
        self.txtarea.insert(END,"\n---------------------------------------")
        self.txtarea.insert(END,f"\n\t\tTotal Kes. {total+(0.16*total)}")
        self.txtarea.insert(END,"\n---------------------------------------")
        self.txtarea.insert(END,"\n\n\n\n\n-----------Thanks for shopping------------")
        self.save_bill()
    ...
    def save_bill(self):
        ...
        f1.close()
        messagebox.showinfo("Saved",f"Receipt No. :{self.bill_no.get()} saved Successfully.")

    def welcome_bill(self):
        self.txtarea.delete('1.0',END)
        self.txtarea.insert(END,"\tWelcome to Danny Academy")
        self.txtarea.insert(END,f"\n Receipt Number: {self.bill_no.get()}")
        self.txtarea.insert(END,f"\n Student Name: {self.c_name.get()}")
    ...
    def find_bill(self):
        ...
        rows=cur.fetchall()
        if len(rows)==0:
            messagebox.showinfo("Not Found",f"Receipt No. :{self.search_bill.get()} not found.")
            return
        inv_date=rows[0][1]
        inv_amount=rows[0][2]
        inv_id=rows[0][0]
        ...
        self.txtarea.delete('1.0',END)
        self.txtarea.insert(END,"\tWelcome to Danny Academy")
        self.txtarea.insert(END,f"\n Receipt Number: {inv_id}")
        self.txtarea.insert(END,f"\n Receipt Date: {inv_date}")
        self.txtarea.insert(END,f"\n Student Name: {c_name}")
